...
export class ChallanService {
  ...
  static async create(userId: string, data: CreateChallanInput) {
    ...
    // Fetch products to build snapshots & calculate total quantity
    ...
    // If requested status is CONFIRMED immediately, execute inside transaction
      ...
      return prisma.$transaction(async (tx) => {
        ...
        for (const item of itemsWithSnapshots) {
          ...
          if (!prod || prod.currentStock < item.quantity) {
            ...
            throw {
              statusCode: 400,
              message: `Insufficient stock for product '${item.productNameSnapshot}' (SKU: ${item.skuSnapshot}). Available: ${avail}, Requested: ${item.quantity}`,
            };
          }
        }
    ...
    return prisma.$transaction(async (tx) => {
      ...
      for (const item of challan.items) {
        ...
        if (!product || product.currentStock < item.quantity) {
          ...
          throw {
            statusCode: 400,
            message: `Insufficient stock for product '${item.productNameSnapshot}' (SKU: ${item.skuSnapshot}). Available stock: ${avail}, Required: ${item.quantity}`,
          };
        }
      }

      // Deduct stock and record OUT movements
      for (const item of challan.items) {
        await tx.product.update({
          where: { id: item.productId },
          data: { currentStock: { decrement: item.quantity } },
